...
if __name__ == "__main__":
    ...
    for agent_name in agent_types:
        for action_name in actions:
            # if agent_name == "TD3":
            #     eligible_rewards = {k: v for k, v in reward_functions.items() if k in EXEC_REWARDS}
            # else:
            #     eligible_rewards = {k: v for k, v in reward_functions.items() if k not in EXEC_REWARDS}

            for reward_name, rf in reward_functions.items():
